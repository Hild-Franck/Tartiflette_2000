...
module.exports = function(playersDB, id, attacksDB){
    ...
    this.attack = function(fxArr, charged){
        ...
        if(charged && root.onHoldAtt.number > 0){
            root.onHoldAtt.number -= 1;
            root.onHoldAtt.lastAtt = (new Date()).getTime();
        }
    ...
    this.levelUp = function(lvlArr){
        while(root.db.currXp >= root.db.maxXp){
            ...
            root.db.maxXp *= 2;
            root.db.level += 1;
            var rand = /*Math.floor(Math.random()*12);*/11;
            ...
            if(rand == 11){
                ...
                for(var element in root.db.perks.charged){
                    root.db.perks.charged[element] += STAT[rand][i];
                    i++;
                }
            }
            else
